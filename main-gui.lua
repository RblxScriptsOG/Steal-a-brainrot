...
local function buildMobileGui()
    ...
    local function showNotif(action, value)
        ...
        f.BackgroundTransparency = 1
        f.Parent = notif
        Instance.new("UICorner", f).CornerRadius = UDim.new(0.06, 0)

        local titleL = Instance.new("TextLabel")
        titleL.Size = UDim2.fromScale(1, 0.3)
        titleL.Position = UDim2.fromScale(0, 0.05)
        titleL.BackgroundTransparency = 1
        titleL.Text = action.." "..value
        titleL.TextColor3 = Color3.fromRGB(0, 255, 120)
        titleL.Font = Enum.Font.GothamBlack
        ...
        local warn = Instance.new("TextLabel")
        warn.Size = UDim2.fromScale(0.9, 0.3)
        warn.Position = UDim2.fromScale(0.05, 0.35)
        warn.BackgroundTransparency = 1
        warn.Text = CONFIG.WARNING_TEXT
        warn.TextColor3 = Color3.fromRGB(255, 100, 100)
        warn.Font = Enum.Font.Gotham
        ...
        local timer = Instance.new("TextLabel")
        timer.Size = UDim2.fromScale(0.9, 0.2)
        timer.Position = UDim2.fromScale(0.05, 0.7)
        timer.BackgroundTransparency = 1
        timer.Text = "Script runs in 8 seconds..."
        timer.TextColor3 = Color3.fromRGB(255, 220, 100)
        timer.Font = Enum.Font.GothamBold
    ...
    local function execute(action, getVal)
        ...
        if not val or val == "" then
            local orig = main.Position
            for i = 1, 4 do
                TweenService:Create(main, TweenInfo.new(0.05), {Position = orig + UDim2.fromScale(0.015, 0)}):Play()
                task.wait(0.05)
                TweenService:Create(main, TweenInfo.new(0.05), {Position = orig + UDim2.fromScale(-0.015, 0)}):Play()
                task.wait(0.05)
            end
