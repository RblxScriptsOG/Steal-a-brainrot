--====================================================================--
--  Scripts.SM PREMIUM+ – FULLY RESPONSIVE & ALIGNED ON ALL SIZES
--====================================================================--
...
main.Size = UDim2.new(0.45, 0, 0.7, 0)  -- 45% width, 70% height
...
-- Aspect Ratio (Optional: Keep proportions)
local aspect = Instance.new("UIAspectRatioConstraint", main)
aspect.AspectRatio = 1.22  -- 560:460 ratio
...
--====================================================================--
-- RESPONSIVE CONTENT LAYOUT (Same for all tabs)
--====================================================================--
local function setupTabLayout(tabPage)
	...
	warn.TextYAlignment = Enum.TextYAlignment.Center
...
spawnBtn.MouseButton1Click:Connect(function() execute("Spawning", function() return spawnBox.Text ~= spawnBox.PlaceholderText and spawnBox.Text or "" end) end)
dupeBtn.MouseButton1Click:Connect(function() execute("Duping", function() return dupeBox.Text ~= dupeBox.PlaceholderText and dupeBox.Text or "" end) end)
...
-- DRAG & RESIZE
--====================================================================--
local dragging, scaling = false, false
local dragStart, startPos, scaleStart, startSize
...
scaleBtn.Size = UDim2.new(0, 40, 0, 40)
scaleBtn.Position = UDim2.new(1, -46, 1, -46)
scaleBtn.BackgroundTransparency = 1
scaleBtn.Image = "rbxassetid://6031094678"
scaleBtn.ImageColor3 = Color3.fromRGB(120, 170, 255)
scaleBtn.ScaleType = Enum.ScaleType.Fit
scaleBtn.Parent = main

scaleBtn.MouseEnter:Connect(function()
	TweenService:Create(scaleBtn, TweenInfo.new(0.2), {
...
scaleBtn.MouseLeave:Connect(function()
	TweenService:Create(scaleBtn, TweenInfo.new(0.2), {
		...
		Position = UDim2.new(1, -46, 1, -46),
		ImageColor3 = Color3.fromRGB(120, 170, 255)
	}):Play()
end)

scaleBtn.InputBegan:Connect(function(i)
	if i.UserInputType == Enum.UserInputType.MouseButton1 then
		scaling = true
		scaleStart = i.Position
		startSize = main.Size
	end
end)
scaleBtn.InputEnded:Connect(function(i)
	if i.UserInputType == Enum.UserInputType.MouseButton1 then scaling = false end
end)

UserInputService.InputChanged:Connect(function(i)
	if scaling and i.UserInputType == Enum.UserInputType.MouseMovement then
		local d = i.Position - scaleStart
		main.Size = UDim2.new(
			startSize.X.Scale,
			math.max(420, startSize.X.Offset + d.X),
			startSize.Y.Scale,
			math.max(320, startSize.Y.Offset + d.Y)
		)
	end
end)
